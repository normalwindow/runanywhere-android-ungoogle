...
import xyz.normalwindow.runanywhere.data.benchmark.CpuSampler
import xyz.normalwindow.runanywhere.data.benchmark.DeviceMonitor
import xyz.normalwindow.runanywhere.data.benchmark.DiskSpeed
...
fun DeviceMonitorSection() {
    ...
    Surface(
        ...
        Column(
            ...
            Text(AppLocale.text("Live monitor"), style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onSurfaceVariant)

            MetricGraph(
                label = AppLocale.text("Memory"),
            ...
            Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(dimens.spacingMd)) {
                Column(modifier = Modifier.weight(1f)) {
                    Text(AppLocale.text("Disk R/W"), style = MaterialTheme.typography.bodyMedium)
                    val speed = diskSpeed
                    Text(
                        text = when {
                            measuring -> AppLocale.text("Measuring…")
                            speed != null -> "W ${"%.0f".format(speed.writeMbps)} · R ${"%.0f".format(speed.readMbps)} MB/s"
                            else -> AppLocale.text("Not measured")
                        },
                ...
                OutlinedButton(
                    ...
                    if (measuring) {
                        ...
                        Text(AppLocale.text("Measure"), modifier = Modifier.padding(start = dimens.spacingSm))
                    }
                }
            }
        }
...
private fun StorageBar(used: Long, total: Long, free: Long) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(dimens.spacingXs)) {
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            Text(AppLocale.text("Storage"), style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
