...
fun TtsScreen() {
    ...
    Column(
        ...
        OutlinedTextField(
            ...
            label = { Text(AppLocale.text("Text to speak")) },
            minLines = 4,
            maxLines = 10,
        )
        ...
        TextButton(onClick = ttsVm::surpriseMe) {
            ...
            Text(AppLocale.text("Surprise me"), modifier = Modifier.padding(start = dimens.spacingSm))
        }

        SliderRow(AppLocale.text("Speed"), String.format(Locale.US, "%.1f×", ttsVm.speed), ttsVm.speed, ttsVm::onSpeedChange)
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(dimens.spacingMd)) {
            OutlinedButton(
                ...
                Text(
                    text = if (ttsVm.isGenerating) AppLocale.text("Generating…") else AppLocale.text("Generate"),
            ...
            Button(
                ...
                Text(
                    text = if (ttsVm.isSpeaking) AppLocale.text("Stop") else AppLocale.text("Speak"),
                    modifier = Modifier.padding(start = dimens.spacingSm),
                )
            }
        }
    ...
    Surface(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(AppLocale.text("Voice"), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                Text(
                    text = voiceName ?: AppLocale.text("Select a voice"),
