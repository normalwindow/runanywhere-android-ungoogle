...
fun CloudProvidersScreen() {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            Text(
                AppLocale.text(HybridBetaCopy.CLOUD_PROVIDERS_EXPLANATION),
            ...
            OutlinedButton(
                ...
                Text(AppLocale.text("Add provider"), modifier = Modifier.padding(start = dimens.spacingSm))
            }
        }
    }
        ...
        Row(
            ...
            IconButton(onClick = onDelete) {
                Icon(
                    ...
                    contentDescription = AppLocale.text("Delete"),
...
private fun ProviderSheet(
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            Text(AppLocale.text("Format"), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
            ...
            OutlinedTextField(
                ...
                label = { Text(AppLocale.text("Name")) }, singleLine = true, modifier = Modifier.fillMaxWidth(),
            )
            OutlinedTextField(
                value = model, onValueChange = { model = it },
                label = { Text(AppLocale.text("Model")) }, singleLine = true, modifier = Modifier.fillMaxWidth(),
            )
            OutlinedTextField(
                value = apiKey, onValueChange = { apiKey = it },
                label = { Text(AppLocale.text("API key")) }, singleLine = true,
            ...
            OutlinedTextField(
                ...
                label = { Text(AppLocale.text("Base URL")) }, singleLine = true,
                ...
                supportingText = if (baseUrl.isNotBlank() && !validBaseUrl) {
                    { Text(AppLocale.text("Enter a valid HTTPS URL without embedded credentials")) }
                } else null,
                modifier = Modifier.fillMaxWidth(),
            )
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(dimens.spacingMd)) {
                TextButton(onClick = onDismiss, modifier = Modifier.weight(1f)) { Text(AppLocale.text("Cancel")) }
                Button(
                    onClick = { onSave(label, preset, model, apiKey, baseUrl) },
                    enabled = canSave,
                    modifier = Modifier.weight(1f),
                ) { Text(AppLocale.text("Save")) }
            }
        }
    }
}
