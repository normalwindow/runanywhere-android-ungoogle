...
fun BenchmarkDetailScreen(runId: String) {
    ...
    if (run == null) {
        Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Text(AppLocale.text("Run not found."), style = MaterialTheme.typography.bodyLarge, color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
        return
    }
...
private fun ExportCard(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(dimens.spacingSm)) {
        Button(onClick = onShareCard, modifier = Modifier.fillMaxWidth()) {
            ...
            Text(AppLocale.text("Share result card"), modifier = Modifier.padding(start = dimens.spacingSm))
        }
        Row(horizontalArrangement = Arrangement.spacedBy(dimens.spacingSm)) {
            OutlinedButton(onClick = onCopyMarkdown, modifier = Modifier.weight(1f)) { Text(AppLocale.text("Copy MD")) }
            OutlinedButton(onClick = onCopyJson, modifier = Modifier.weight(1f)) { Text(AppLocale.text("Copy JSON")) }
        }
        Row(horizontalArrangement = Arrangement.spacedBy(dimens.spacingSm)) {
            OutlinedButton(onClick = onCopyCsv, modifier = Modifier.weight(1f)) { Text(AppLocale.text("Copy CSV")) }
            OutlinedButton(onClick = onShare, modifier = Modifier.weight(1f)) {
                Icon(RACIcons.Outline.Send, contentDescription = null, modifier = Modifier.size(dimens.iconSm))
                Text(AppLocale.text("Share text"), modifier = Modifier.padding(start = dimens.spacingSm))
            }
        }
    }
}
