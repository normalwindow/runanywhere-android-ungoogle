...
import xyz.normalwindow.runanywhere.tools.InvalidWebSearchDeviceIdentityException
...
class WebSearchToolTest {
    ...
    fun `query is encoded into fixed HTTPS search origin`() {
        ...
        val bing = WebSearchTool.buildBingSearchUrl("current NPU & Android")
        assertTrue(bing.startsWith("https://www.bing.com/search?"))
        assertTrue(bing.contains("q=current%20NPU%20%26%20Android"))
    }

    @Test
    fun `bing results parse titles snippets and safe source URLs`() {
        val html = """
            <li class="b_algo">
              <h2><a href="https://example.com/hexagon">Hexagon &amp; NPU</a></h2>
              <p class="b_lineclamp">Current <b>guidance</b>.</p>
            </li>
            <li class="b_algo">
              <h2><a href="javascript:alert(1)">Unsafe</a></h2>
              <p class="b_lineclamp">Drop me.</p>
            </li>
        """.trimIndent()

        val result = WebSearchTool.parseBingResults(html).single()

        assertEquals("Hexagon & NPU", result.title)
        assertEquals("Current guidance.", result.snippet)
        assertEquals("https://example.com/hexagon", result.url)
    }

    @Test
    fun `default public search uses Bing without contacting DuckDuckGo`() = runBlocking {
        ...
        val result = WebSearchTool.search(
            ...
            publicFetcher = { url ->
                ...
                """
                <li class="b_algo">
                  <h2><a href="https://example.com/v81">Hexagon V81</a></h2>
                  <p class="b_lineclamp">Fallback response.</p>
                </li>
                """.trimIndent()
            },
        )
        ...
        assertTrue(requestedUrls.single().startsWith("https://www.bing.com/search?"))
        assertEquals("Fallback response.", result.getValue("summary").string)
        assertEquals("https://example.com/v81", result.getValue("source_url").string)
    }
